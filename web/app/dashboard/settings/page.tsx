...
const inputClass =
  'w-full rounded-lg border border-stone-700 bg-stone-800 px-3 py-2 text-sm text-stone-200 placeholder:text-stone-600 focus:border-blue-500 focus:outline-none'
  ...
  return (
    <div className="space-y-8">
      <header>
        ...
        <p className="mt-1 text-sm text-stone-500">Manage your workspace profile and subscription plan.</p>
      </header>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card>
          <CardHeader className="flex flex-wrap items-center justify-between gap-3">
            <h2 className="text-base font-semibold text-stone-100">Workspace profile</h2>
            {workspaces.length > 1 ? (
              <select
                value={selectedId}
        ...
        <Card>
          <CardHeader className="flex items-center justify-between">
            <h2 className="text-base font-semibold text-stone-100">Billing &amp; plan</h2>
          ...
          <CardBody className="space-y-5">
            <div className="rounded-xl border border-stone-800 bg-stone-950/40 p-4">
              <div className="flex items-baseline justify-between">
                <span className="text-sm font-medium text-stone-200">{plan.name || (isPro ? 'Pro' : 'Free')} plan</span>
            ...
            {!stripeEnabled ? (
              <div className="rounded-lg border border-stone-800 bg-stone-950/40 p-3 text-sm text-stone-400">
      ...
      <Card>
        <CardHeader>
          <h2 className="text-base font-semibold text-stone-100">Your workspaces</h2>
        </CardHeader>
            ...
            <ul className="divide-y divide-stone-800">
              {workspaces.map((w) => (
                <li key={w.id} className="flex items-center justify-between px-5 py-3">
                  <div>
                    <div className="font-medium text-stone-100">{w.name}</div>
                    {w.legal_name ? <div className="text-xs text-stone-500">{w.legal_name}</div> : null}
                  </div>
                  <button
                    onClick={() => onSelectWorkspace(w.id)}
                    className={`text-xs font-medium ${
                      w.id === selectedId ? 'text-blue-300' : 'text-stone-500 hover:text-stone-300'
                    }`}
                  >
